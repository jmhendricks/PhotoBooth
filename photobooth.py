...
addr_to   = ' seceretcode@tumblr.com' # The special tumblr auto post email address
addr_from = 'youremail@gmail.com' # change to your full gmail address
user_name = 'yourusername' # change to your gmail username
password = 'andyourpassword' # change to your gmail password
test_server = 'www.google.com'

#set the size of the photo paper
w = 800
h = 480
...
def is_connected():
  try:
    ...
    return True #**************Change to true when ready for procduction
  except:
     pass
  return False    
...
# define which printer to use swap between a few printers because they are slow 90s to print one image		
...
def start_photobooth(): 
	...
	try: #take the photos
		for i, filename in enumerate(camera.capture_continuous(file_path + now + '-' + '{counter:02d}.jpg')):
			...
			print("pause in-between shots")
			if i == total_pics-1:
				break
	finally:
		GPIO.output(led2_pin,False) #turn on the LED
		GPIO.output(led5_pin,False)
		camera.stop_preview()
		camera.close()
	########################### Begin Step 3 #################################
	...
	os.system('montage /home/pi/photobooth/pics/*.jpg -tile 2x2 -geometry +10+10 /home/pi/temp_montage2.jpg')  
	#print "made montage and now sending to printer =" +whichprinter()+""
	os.system('lp -d ' + whichprinter() + ' /home/pi/temp_montage2.jpg')
	#print "made montage and now sending to printer"
	graphicsmagick = "gm convert -delay " + str(gif_delay) + " " + file_path + now + "*.jpg " + file_path + now + ".gif" 
	...
	pygame.quit()
	print "Done"
	os.system('suffix=$(date +%H%M%S)')
	os.system('cp /home/pi/temp_montage2.jpg /home/pi/photobooth/PB_archive/PB_$(date +%H%M%S).jpg')
	os.system('rm /home/pi/photobooth/pics/*.jpg')
	os.system('rm /home/pi/photobooth/pics/*.gif')
	os.system('rm /home/pi/temp_montage2.jpg')
...
GPIO.add_event_detect(button2_pin, GPIO.FALLING, callback=exit_photobooth, bouncetime=300) #use second button to exit python. Good while developing
...
while True:
	...
	print "Trying to run the booth again..."
	start_photobooth()
